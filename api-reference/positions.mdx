...
| --- | --- |
| Open market (not claimable) | `value ≈ shares × yesPrice` if `side` is `yes`, else `shares × noPrice` |
| Claimable win | Treat winning shares ≈ **1 USDC each** (or use `winningShares` from [Build win claim](/api-reference/claims/build)) |
| Resolved loser | ≈ `0` |
