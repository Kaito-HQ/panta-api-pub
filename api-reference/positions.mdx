...
Indexer balances may lag the chain briefly after a purchase. Result sets are capped (typically 200 rows). When `claimable` is `true`, use [Build claim](/api-reference/claims/build).
...
<RequestExample>
```bash cURL
curl "https://api.panta.market/api/v1/positions/?wallet=Buyer111111111111111111111111111111111" \
