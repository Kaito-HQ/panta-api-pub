...
description: Validate parameters, check the market image URL, reserve a create session, and return the USDC creation fee.
...
`imageUrl` is required. Host the file yourself (S3, GCS, R2, Cloudinary, …) with public HTTPS read. The API fetches it at quote time.

| Rule | Value |
| --- | --- |
| Count | Exactly one URL (string, not an array) |
| Dimensions | Exactly **1024×1024** |
| Formats | JPG / JPEG, PNG, GIF, or WebP |
| Max size | 5 MB |
| Protocol | Publicly reachable `https` (or `http`) |

Do not pass data URLs, short-lived signed URLs, or private buckets.
...
<ParamField body="imageUrl" type="string" required>
  Public URL of a 1024×1024 JPG/PNG/GIF/WebP (≤5 MB). Catalog only.
</ParamField>
...
Errors: `INVALID_MARKET_PARAMS` · `DUPLICATE_MARKET` · `CREATE_NOT_PERMITTED` · `UNAUTHORIZED` · `RATE_LIMITED`. A bad `imageUrl` is HTTP `400` with field errors.

<RequestExample>
```bash cURL
curl -X POST https://api.panta.market/api/v1/markets/create/quote/ \
