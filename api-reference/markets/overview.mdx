...
<Info>
Every quote needs a catalog `imageUrl` (public `http`/`https`, no localhost/private hosts). **1024×1024** square is recommended for cards; the API does not hard-check dimensions or file size. Optional: `region`, `oracle`, and `eventInProgress` (breaking only). Details on the [quote](/api-reference/markets/quote) page.
</Info>
