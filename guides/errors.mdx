...
Switch on `code`. HTTP status is the class of failure, not a replacement for `code`.
...
| --- | --- | --- |
| `UNAUTHORIZED` | Any | Invalid or missing API key; or wallet does not match the bound session |
| `CREATE_NOT_PERMITTED` | Create market | Account lacks create permission |
...
Some validation failures (for example a bad `imageUrl`) return HTTP `400` with field errors instead of a single `code`.
...
| `register` | Register, trade report, submit | 40 / 60s |
